...
int BD_Data,zero_value,sensitive_value;
...
//	{
		...
		BD_setLow(scl_gpio);
		count=0; 
		while(gpio_in_read(sda_gpio_in)); 
		for(i=0;i<24;i++)
		{ 
			//HX711_SCK=1; 
			BD_setHigh(scl_gpio);
...
{
    if((BD_i2c_read()-zero_value)>sensitive_value)
		return 1;
	else
		return 0;
}
...
command_BDendstop_home(uint32_t *args)
{
	...
	if(args[9]==1)
	{
        zero_value=BD_i2c_read();
		sensitive_value=args[10];
		switch_mode=1;
	}
}
