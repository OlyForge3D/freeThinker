...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        main_title_label.set_markup(
            f"<span font_desc='Sans Bold 24'>{_('Statistics')}</span>"
        )
        ...
        self.create_stat_row(
            stats_grid, _("Total Print Time"), "Loading...", 0, "total_print_time"
        )
        self.create_stat_row(
            stats_grid, _("Longest Print Time"), "Loading...", 1, "longest_print"
        )
        self.create_stat_row(
            stats_grid, _("Average Print Time"), "Loading...", 2, "average_print_time"
        )
        self.create_stat_row(
            stats_grid, _("Total Filament Used"), "Loading...", 3, "total_filament_used"
        )
    ...
    def _update_ui_totals(self, job_totals):
        ...
        self.labels["longest_print"].set_text(
            self._format_time(job_totals.get("longest_print", 0))
        )
        self.labels["total_filament_used"].set_text(
            f"{job_totals.get('total_filament_used', 0) / 1000:.1f} m"
        )
    ...
    def _update_ui_chart(self, history_result):
        ...
        self.chart_data = [
            {
                "label": _("Finished"),
                "value": status_counts["finish_printing"],
                "color": (0.2, 0.7, 0.3),
            },
            {
                "label": _("Cancelled"),
                "value": status_counts["unprint"],
                "color": (0.9, 0.3, 0.2),
            },
            {
                "label": _("Klippy Close"),
                "value": status_counts["klippy_closed"],
                "color": (0.2, 0.6, 0.9),
            },
            {
                "label": _("Other"),
                "value": status_counts["other"],
                "color": (0.6, 0.6, 0.6),
            },
        ]
        self.drawing_area.queue_draw()
    ...
    def create_stat_row(self, grid, name, value, row, data_key):
        ...
        name_label.get_style_context().add_class("stat-label")
    ...
    def on_draw_chart(self, da, ctx):
        ...
        total_drawable_angle = (
            2 * math.pi - (num_segments * gap_angle)
            if num_segments > 1
            else 2 * math.pi
        )
        ...
        for item in self.chart_data:
            ...
            start_x, start_y = (
                math.cos(label_angle) * line_start_radius,
                math.sin(label_angle) * line_start_radius,
            )
            end_x, end_y = (
                math.cos(label_angle) * line_end_radius,
                math.sin(label_angle) * line_end_radius,
            )
            ...
            label_infos.append(
                {
                    ...
                    "box": [text_x, text_y, text_width, text_height],
                    "original_y": end_y,
                }
            )
        ...
        while running and iteration < max_iterations:
            ...
            for i in range(len(label_infos) - 1):
                ...
                if box1[1] < box2[1] + box2[3] and box2[1] < box1[1] + box1[3]:
                    if label_infos[i]["is_right"] != label_infos[i + 1]["is_right"]:
                        continue
                    running = True
