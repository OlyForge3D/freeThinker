...
class Panel(ScreenPanel):
    def __init__(self, screen, title):
        ...
        for extruder in self._printer.get_tools():
            if self._printer.extrudercount > 1:
                self.labels[extruder] = self._gtk.Button(
                    f"extruder-{i}", f"T{self._printer.get_tool_number(extruder)}"
                )
            else:
    ...
    def change_temp(self, widget, temp):
        ...
        self.button_pre_heat_1.get_style_context().remove_class("distbutton_active")
        self.button_pre_heat_2.get_style_context().remove_class("distbutton_active")
        self.buttons["temperature"].get_style_context().remove_class(
            "distbutton_active"
        )
    ...
    def process_update(self, action, data):
        ...
        for x in self._printer.get_tools():
            ...
            if target > 190 and target != self.target_old:
                ...
                self.buttons["temperature"].get_style_context().add_class(
                    "distbutton_active"
                )
                self.button_pre_heat_0.get_style_context().remove_class(
                    "distbutton_active"
                )
                self.button_pre_heat_1.get_style_context().remove_class(
                    "distbutton_active"
                )
                self.button_pre_heat_2.get_style_context().remove_class(
                    "distbutton_active"
                )
    ...
    def extrude(self, widget, direction):
        ...
        self._screen._ws.klippy.gcode_script(
            KlippyGcodes.extrude(f"{direction}{self.distance}", f"{self.speed * 60}")
        )
    ...
    def load_unload(self, widget, direction):
        ...
        if direction == "-":
            ...
            if not self.unload_filament:
                ...
                self._screen._ws.klippy.gcode_script(
                    f"UNLOAD_FILAMENT  NOZZLE_TEMP={self.load_temp}"
                )
        if direction == "+":
            ...
            if not self.load_filament:
                ...
                self._screen._ws.klippy.gcode_script(
                    f"LOAD_FILAMENT  NOZZLE_TEMP={self.load_temp}"
                )
